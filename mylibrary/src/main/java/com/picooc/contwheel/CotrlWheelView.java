...
public class CotrlWheelView extends View
{
  ...
  private DataSetObserver dataObserver = new DataSetObserver()
  {
    ...
    {
      CotrlWheelView.this.invalidateWheel(false);
    }

    public void onInvalidated()
    {
      CotrlWheelView.this.invalidateWheel(true);
    }
  };
  ...
  private CotrlWheelRecycle recycle = new CotrlWheelRecycle(this);
  private CotrlWheelScroller scroller;
  CotrlWheelScroller.ScrollingListener scrollingListener = new CotrlWheelScroller.ScrollingListener()
  {
    public void onFinished()
    {
      if (CotrlWheelView.this.isScrollingPerformed)
      {
        CotrlWheelView.this.notifyScrollingListenersAboutEnd();
        CotrlWheelView.this.isScrollingPerformed = false;
      }
      CotrlWheelView.this.scrollingOffset = 0;
      CotrlWheelView.this.invalidate();
    }

    public void onJustify()
    {
      if (Math.abs(CotrlWheelView.this.scrollingOffset) > 1)
        CotrlWheelView.this.scroller.scroll(CotrlWheelView.this.scrollingOffset, 0);
    }

    public void onScroll(int paramAnonymousInt)
    {
      CotrlWheelView.this.doScroll(paramAnonymousInt);
      if (CotrlWheelView.this.scrollingOffset > 50)
      {
        CotrlWheelView.this.scrollingOffset = 50;
        CotrlWheelView.this.scroller.stopScrolling();
      }
      while (CotrlWheelView.this.scrollingOffset >= -50)
        return;
      CotrlWheelView.this.scrollingOffset = (-50);
      CotrlWheelView.this.scroller.stopScrolling();
    }
    ...
    {
      CotrlWheelView.this.isScrollingPerformed = true;
      CotrlWheelView.this.notifyScrollingListenersAboutStart();
    }
  };
  ...
  public CotrlWheelView(Context paramContext)
  {
    super(paramContext);
    initData(paramContext);
  }

  public CotrlWheelView(Context paramContext, AttributeSet paramAttributeSet)
  {
    super(paramContext, paramAttributeSet);
    initData(paramContext);
  }

  public CotrlWheelView(Context paramContext, AttributeSet paramAttributeSet, int paramInt)
  {
  ...
  private void initResourcesIfNecessary()
  {
    if (this.text == null)
    {
      this.centerDrawable = getContext().getResources().getDrawable(2130838673);
      Log.i("qianmo3", "one--------------------");
    }
